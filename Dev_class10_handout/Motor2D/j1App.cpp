...
void j1App::FinishUpdate()
{
	...
	SDL_Delay(wait_time);         // dt between the expected 16 ms and the actual time 
	PERF_PEEK(ptimer);
	
	LOG("Wait time until frame is finished: %i", wait_time); 


	dt = ptimer.ReadMs() / 1000; // to capture a single frame time in seconds
	LOG("Lrame time ----> %f", dt); 
